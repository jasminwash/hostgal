...
from transfer_to_result import transfer_to_result, transfer_obj_to_result, transfer_to_result_ps
...
#goes [y,x]    
    ...
    source_result, ps_result, image_ps, image_host, error_map,rx3=fit_qso(QSO_img, psf_ave=psf, psf_std = None,
                                                                     source_params=[], QSO_msk = QSO_msk, fixcenter=fixcenter,
                                                                     pix_sz = pix_scale, no_MCMC = (run_MCMC==False),
                                                                     QSO_std =QSO_std, tag=tag, deep_seed= deep_seed, pltshow=pltshow,
                                                                     corner_plot=False, flux_ratio_plot=True, dump_result=run_MCMC,band='PSonly',return_Chisq=True)
#    print("Blues")
#    print(rx3)
    if pltshow == 0:
        plot_compare=False
...
#    print(np.unique(image_ps))
    ...
    print("Source")
    print(source_result)
    result,c2mask= transfer_to_result_ps(data=QSO_img, pix_sz = pix_scale,
                                ps_result=ps_result, image_ps=image_ps, image_host=image_host, error_map=error_map,
                                fixcenter=fixcenter,ID='Example', QSO_msk = QSO_msk, tag=tag, plot_compare = plot_compare,zp=zp, band='PSonly')
    #if QSO_msk is None:
    #    QSO_mask = QSO_img*0 + 1
    #dmps=(flux_list[0]-flux_list[1])*QSO_mask
    #hdu=fits.PrimaryHDU(dmps)
    #hdu.writeto('/Users/jasminwashington/Google Drive File Stream/My Drive/usrp/git_repo/hostgal/py_tools/targets/' + name + '/'+name+'I.fits',overwrite=True)
    return result,c2mask
    ''' 
